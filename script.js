...
function compute(event){
    ...
    if ((amount.value && amount.value > 0) && (rate.value && rate.value <= 100 ) && (years.value && years.value > 0) ){
        let date = new Date();

        result.innerHTML = `If you deposit <span class="span_yellow">${"$" + amount.value}</span>, <br>
                            at an interest rate of <span class="span_yellow">${rate.value}%</span>, <br>
                            You will receive an amount of <span class="span_yellow">${"$" + parseInt(years.value)*parseInt(amount.value)*(parseFloat(rate.value)/100)}</span>, <br>
                            in the year <span class="span_yellow">${date.getFullYear() + parseInt(years.value)}</span>
                            `;
